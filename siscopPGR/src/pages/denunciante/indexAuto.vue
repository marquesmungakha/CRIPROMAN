<template>
  <q-page class="q-pa-sm q-gutter-sm">
    <q-table :columns="columns" :data="allDenunciantesFromAutoEntrada" :filter="filter" binary-state-sort
    ...
    <div class="q-pa-sm q-gutter-sm">
            ...
            <div class="q-pa-md">
              ...
              <individuo :apelido.sync="denunciante.apelido"
                    ...
                    <q-step
                      ...
                      icon="assignment">
      <create-edit-form :descricaoOcorrencia.sync="autoEntradaDenunciante.descricaoOcorrencia"
                        :localTrabalho.sync="autoEntradaDenunciante.localTrabalho"
                        :ocupacao.sync="autoEntradaDenunciante.ocupacao"
                        :qualidadeDe.sync="autoEntradaDenunciante.qualidadeDe"/>
                         </q-step>
     ...
     <details-denunciante :denunciante.sync="denunciante" 
                       :image.sync="image" 
                       :autoEntradaDenunciante.sync="autoEntradaDenunciante"
                       :tipoDocumento.sync="tipoDocumento"
                       :pais.sync="pais" 
...
import AutoEntradaDenunciante from 'src/store/models/autoEntrada/autoEntradaDenunciante'
...
export default {
  ...
  metaInfo: {},
  computed: {
    allDenunciantesFromAutoEntrada() {
      return AutoEntrada.query().with('denunciantes')
                                 ...
                                 .find(this.autoEntrada.id).denunciantes
    },
    allPaises() {
  ...
  methods: {
    ...
    createDenunciante() {
      ...
      this.denunciante.autoEntrada_id = this.autoEntrada.id
      ...
      this.denunciante.autoEntrada = this.autoEntrada

      this.autoEntradaDenunciante.denunciante =  this.denunciante
      this.autoEntradaDenunciante.autoEntrada = this.autoEntrada
     
     console.log(this.autoEntradaDenunciante)
      ...
      if (this.editedIndex > -1) {
         AutoEntradaDenunciante.api().patch("/autoEntradaDenunciante/" + this.autoEntradaDenunciante.id, this.autoEntradaDenunciante).then(resp => {
          this.$q.notify({
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi actualizada com successo!! [' + this.autoEntradaDenunciante.denunciante.nome + ' ]'
          })
          this.close()
        }).catch(error => {
        ...
        AutoEntradaDenunciante.api().post("/autoEntradaDenunciante/", this.autoEntradaDenunciante).then(resp => {
          ...
          this.$q.notify({
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi inserida com successo! [ ' + this.autoEntradaDenunciante.denunciante.nome + ' ]'
          })
          this.close()
        }).catch(error => {
          console.log('Erro no code ' + error)
    ...
    removeDenunciante(denunciante) {
      this.$q.dialog({
        ...
        this.$q.notify({
         ...
         AutoEntradaDenunciante.delete(this.utoEntradaDenunciante.id)
      })
    },
    editaDenunciante(denunciante) {
      this.editedIndex = 0
      this.autoEntradaDenunciante = Object.assign({}, denunciante)
      this.denunciante =  this.autoEntradaDenunciante.denunciante
      this.pais = Pais.query().find(this.denunciante.nacionalidade_id)
      this.provincia = Provincia.query().find(this.denunciante.provincia_id)
      this.tipoDocumento = TipoDocumentoIdentificacao.query().find(this.denunciante.tipoDocumento_id)
      this.image ='data:image/jpeg;base64,' + btoa(new Uint8Array(this.denunciante.fotografia).reduce((data, byte) => data + String.fromCharCode(byte), ''))
    ...
    mostraDenunciante(denunciante) {
      this.editedIndex = 0
      this.autoEntradaVitima = Object.assign({}, denunciante)
      this.denunciante =  this.autoEntradaVitima.denunciante
      this.pais = Pais.query().find(this.denunciante.nacionalidade_id)
      this.provincia = Provincia.query().find(this.denunciante.provincia_id)
      this.tipoDocumento = TipoDocumentoIdentificacao.query().find(this.denunciante.tipoDocumento_id)
      this.image ='data:image/jpeg;base64,' + btoa(new Uint8Array(this.denunciante.fotografia).reduce((data, byte) => data + String.fromCharCode(byte), ''))
   ...
   getAllTipoDocumentoIdentificacao(offset) {
    ...
    exportTable() {
      ...
      const content = [this.columns.map(col => wrapCsvValue(col.label))]
        .concat(
          this.allDenunciantesFromAutoEntrada.map(row =>
            this.columns
              .map(col =>
                wrapCsvValue(
