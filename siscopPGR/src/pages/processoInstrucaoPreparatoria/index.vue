...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'accoesCrimes', align: 'left', label: 'Ações/Crimes', field: row => row.accoesCrimes, format: val => `${val}`, sortable: true },
  ...
  computed: {
    ...
    allProcessos () {
      return ProcessoInstrucaoPreparatoria.query()
                                          ...
                                          .where('orgao_id',Number(localStorage.getItem('orgaoId'))).get()
    }
  },
  methods: {
    createProcesso () {
 ...
 this.orgao = Orgao.query().with('provincia.*')
      ...
      this.processoInstrucaoPreparatoria.orgao = this.orgao
      this.processoInstrucaoPreparatoria.orgao_id = this.orgao.id
    ...
    async getAllProcesso(offset) {
      if(offset >= 0) {
         await ProcessoInstrucaoPreparatoria.api().get("/processoInstrucaoPreparatoria?offset="+offset+"&max=100").then(resp => {
          offset = offset + 100
  ...
  async getAllPais(offset) {
    ...
    async getAllParecerProcesso(offset) {
      if(offset >= 0){
          await ParecerProcesso.api().get("/parecerProcesso?offset="+offset+"&max=100").then(resp => {
          offset = offset + 100
          if(resp.response.data.length > 0) 
