<template>
  <q-page class="q-pa-sm q-gutter-sm">
    <q-table :columns="columns" :data="allVitimasFromProcessoInstrucaoPreparatoria" :filter="filter" binary-state-sort
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td key="naturalidade" :props="props">
            {{ props.row.vitima.naturalidade }}
            <q-popup-edit v-model="props.row.vitima.naturalidade">
              <q-input v-model="props.row.vitima.naturalidade" autofocus counter dense></q-input>
            </q-popup-edit>
          </q-td>
          ...
          <q-td key="localNascimento" :props="props">
            {{ props.row.vitima.localNascimento }}
            <q-popup-edit v-model="props.row.vitima.localNascimento">
              <q-input v-model="props.row.vitima.localNascimento" autofocus counter dense></q-input>
            </q-popup-edit>
          </q-td>
          ...
          <q-td key="documentoValidade" :props="props">
            {{ props.row.vitima.documentoValidade }}
            <q-popup-edit v-model="props.row.vitima.documentoValidade">
              <q-input v-model="props.row.vitima.documentoValidade" autofocus counter dense></q-input>
            </q-popup-edit>
          </q-td>
          <q-td key="morada" :props="props">
            {{ props.row.vitima.morada }}
            <q-popup-edit v-model="props.row.vitima.morada">
              <q-input v-model="props.row.vitima.morada" autofocus counter dense></q-input>
            </q-popup-edit>
          </q-td>
    ...
    <div class="q-pa-sm q-gutter-sm">
     ...
     <details-vitima :vitima.sync="vitima" 
                       :image.sync="image" 
                       :processoInstrucaoPreparatoriaVitima.sync="processoInstrucaoPreparatoriaVitima" 
                       :tipoDocumento.sync="tipoDocumento"
                       :pais.sync="pais"
...
import ProcessoInstrucaoPreparatoriaVitima from 'src/store/models/processoInstrucaoPreparatoria/processoInstrucaoPreparatoriaVitima'
...
export default {
  ...
  data() {
    return {
      ...
      processoInstrucaoPreparatoriaVitima: {
        declaracao: '',
        vitima: {},
        processoInstrucaoPreparatoria: {}
      },
  ...
  metaInfo: {},
  computed: {
    allVitimasFromProcessoInstrucaoPreparatoria() {
      return ProcessoInstrucaoPreparatoria.query().with('vitimas')
                                 .with('vitimas.vitima')
                                 ...
                                 .find(this.processoInstrucaoPreparatoria.id).vitimas
    },
    allPaises() {
  ...
  methods: {
    ...
    createVitima() {
      ...
      this.vitima.processoInstrucaoPreparatoria_id = this.processoInstrucaoPreparatoria.id
      this.vitima.nacionalidade = this.pais
      this.vitima.provincia = this.provincia
      this.vitima.tipoDocumento = this.tipoDocumento
      this.vitima.processoInstrucaoPreparatoria = this.processoInstrucaoPreparatoria

      this.processoInstrucaoPreparatoriaVitima.vitima =  this.vitima
      this.processoInstrucaoPreparatoriaVitima.processoInstrucaoPreparatoria = this.processoInstrucaoPreparatoria
     
     console.log(this.processoInstrucaoPreparatoriaVitima)
      ...
      if (this.editedIndex > -1) {
         ProcessoInstrucaoPreparatoriaVitima.api().patch("/processoInstrucaoPreparatoriaVitima/" + this.processoInstrucaoPreparatoriaVitima.id, this.processoInstrucaoPreparatoriaVitima).then(resp => {
          this.$q.notify({
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi actualizada com successo!! [' + this.processoInstrucaoPreparatoriaVitima.vitima.nome + ' ]'
          })
          this.close()
        }).catch(error => {
        ...
        ProcessoInstrucaoPreparatoriaVitima.api().post("/processoInstrucaoPreparatoriaVitima/", this.processoInstrucaoPreparatoriaVitima).then(resp => {
          ...
          this.$q.notify({
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi inserida com successo! [ ' + this.processoInstrucaoPreparatoriaVitima.vitima.nome + ' ]'
          })
          this.close()
        }).catch(error => {
          console.log('Erro no code ' + error)
    ...
    editaVitima(vitima) {
      this.editedIndex = 0
      this.processoInstrucaoPreparatoriaVitima = Object.assign({}, vitima)
      this.vitima =  this.processoInstrucaoPreparatoriaVitima.vitima
      this.pais = Pais.query().find(this.vitima.nacionalidade_id)
      this.provincia = Provincia.query().find(this.vitima.provincia_id)
      this.tipoDocumento = TipoDocumentoIdentificacao.query().find(this.vitima.tipoDocumento_id)
      this.image ='data:image/jpeg;base64,' + btoa(new Uint8Array(this.vitima.fotografia).reduce((data, byte) => data + String.fromCharCode(byte), ''))
