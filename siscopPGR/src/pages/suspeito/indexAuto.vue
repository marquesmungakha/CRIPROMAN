<template>
  <q-page class="q-pa-sm q-gutter-sm">
    <q-table :columns="columns" :data="allSuspeitosFromAutoEntrada" :filter="filter" binary-state-sort row-key="name"
             title="Suspeito">
             ...
             <div class="q-pa-md">
              ...
              <individuo :apelido.sync="suspeito.apelido"
                    ...
                    <q-step
                      ...
                      icon="assignment">
              <create-edit-form :dataSituacaoPrisional.sync="autoEntradaSuspeito.dataSituacaoPrisional"
 ...
 <details-suspeito :suspeito.sync="this.suspeito" 
                       :image.sync="image" 
                       :autoEntradaSuspeito.sync="this.autoEntradaSuspeito" 
                       :tipoDocumento.sync="tipoDocumento"
...
import AutoEntradaSuspeito from 'src/store/models/autoEntrada/autoEntradaSuspeito'
...
export default {
  ...
  mounted() {
    ...
    this.getAllProfissaoAutoEntrada(offset)
  ...
  metaInfo: {},
  computed: {
   allSuspeitosFromAutoEntrada() {
      return AutoEntrada.query().with('suspeitos')
                                 ...
                                 .find(this.autoEntrada.id).suspeitos
    },
    allPaises() {
  ...
  methods: {
    ...
    createSuspeito() {
     ...
     this.provincia.pais = this.pais
      ...
      this.suspeito.autoEntrada_id = this.autoEntrada.id
      ...
      this.autoEntradaSuspeito.profissao = this.profissao
      this.autoEntradaSuspeito.profissao_id = this.profissao.id
      this.autoEntradaSuspeito.situacaoPrisional = this.situacaoPrisional
      this.autoEntradaSuspeito.autoEntrada = this.autoEntrada
      this.autoEntradaSuspeito.suspeito = this.suspeito

      if (this.editedIndex > -1) {
         AutoEntradaSuspeito.api().patch("/autoEntradaSuspeito/" + this.autoEntradaSuspeito.id, this.autoEntradaSuspeito).then(resp => {
          this.$q.notify({
            type: 'positive',
            color: 'green-4',
            textColor: 'white',
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi actualizada com successo!! [' + this.autoEntradaSuspeito.suspeito.nome + ' ]'
          })
          this.close()
        }).catch(error => {
        ...
        AutoEntradaSuspeito.api().post("/autoEntradaSuspeito/", this.autoEntradaSuspeito).then(resp => {
          console.log(resp)
          this.$q.notify({
            type: 'positive',
            color: 'green-4',
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi inserida com successo! [ ' + this.autoEntradaSuspeito.suspeito.nome + ' ]'
          })
          this.close()
        }).catch(error => {
          console.log('Erro no code ' + error)
    ...
    removeSuspeito(suspeito) {
      this.$q.dialog({
        ...
        AutoEntradaSuspeito.api().delete("/autoEntradaSuspeito/" + this.autoEntradaSuspeito.id)
         AutoEntradaSuspeito.delete(this.autoEntradaSuspeito.id)
      })
    },
    editaSuspeito(suspeito) {
      this.step = 2
      this.editedIndex = 0
      this.autoEntradaSuspeito = Object.assign({}, suspeito)
      this.suspeito = this.autoEntradaSuspeito.suspeito
      ...
      this.suspeito.autoEntrada = this.autoEntrada
   ...
   getAllProfissaoAutoEntrada(offset) {
      if(offset >= 0){
          AutoEntradaSuspeito.api().get("/autoEntradaSuspeito?offset="+offset+"&max=100").then(resp => {
          offset = offset + 100
          if(resp.response.data.length > 0) 
              setTimeout(this.getAllProfissaoAutoEntrada(offset), 2)
          }).catch(error => {
          console.log('Erro no code ' + error)
        })
       }
