...
export default {
  ...
  mounted () {
   ...
   this.getAllProcesso(offset)
    ...
    this.getDependenciasOrgao(offset)
  ...
  computed: {
    ...
    let listOrgaoId = DependenciaOrgao.query().where('orgao_id',Number(localStorage.getItem('orgaoId'))).get()

    console.log(listOrgaoId)

    if(this.orgao.nivel === 0){
       const findos = ProcessoInstrucaoPreparatoria.query()
                                                   .has('pareceresProcesso',1)
                                                   .withAll()
                                                   .with('arguidos.situacaoPrisional')
    ...
    return processo.concat(transitados).concat(findos)
       ...
       const findos = ProcessoInstrucaoPreparatoria.query()
                                                ...
                                                .with('arguidos.situacaoPrisional')
                                                .with('pareceresProcesso.tipoParecer')
                                                .where('orgao_id', Number(localStorage.getItem('orgaoId')))
                                                // .orWhere((query) => {
                                                //                    if(query !== null && query !== undefined){
                                                //                         return listOrgaoId.map( (orgao) => {
                                                //                                     return orgao.id === query.orgao_id
                                                //                                     } )
                                                //                    }
                                                // })
                                                .all().map(v => ({...v, movimento:'Findos'}))
    ...
    allData () {
      ...
      //       classes: 'glossy',
      //       progress: true,
      //       message: 'Sem informação para apresentar!'
      ...
      let situacaoPrisional = null
       ...
       if(processo.arguidos.length > 0){
          situacaoPrisional = processo.arguidos.map( (arguido) => {return arguido.situacaoPrisional})
            ...
            data.push(
          {
            ...
            'Tipo Parecer': (processo.pareceresProcesso.length > 0) ? TipoParecer.query().find(processo.pareceresProcesso[0].tipoParecer_id).designacao : ' ',
            'Destino': (processo.pareceresProcesso.length > 0) ? Orgao.query().find(processo.pareceresProcesso[0].destino_id).designacao : ' ',
            'Orgão': (processo !== null) ? Orgao.query().find(processo.orgao_id).designacao : ' ', 
            'Jurisdição/Familia Delitiva': (processo !== null) ? ClasseJudicial.query().find(processo.classeJudicial_id).designacao : ' ',
            'Ações/Crimes': (processo !== null) ? Crime.query().find(processo.accoesCrimes_id).designacao : ' ',
