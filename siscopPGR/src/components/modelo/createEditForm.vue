<template>
    <div class="q-pa-sm q-gutter-sm">
       <q-dialog v-model="show_dialog" persistent>
        ...
        <q-card-section>
            <div class="text-h6">Adicionar  Modelo!</div>
        </q-card-section>
                <q-card-section>
        ...
        <q-card-section style="max-height: 50vh" class="scroll">
                <q-form @submit.prevent="createModelo" class="q-gutter-md">
                    <q-list>
                        <div class="row">                
        <div class="row q-gutter-xs" style="width: 500px;">
          <div class="col-4 text-left">Código *</div>
                                ...
                                lazy-rules
                                :rules="[ val => val && val.length > 0 || 'Introduza o codigo do Modelo']" />
        </div>           
        <div class="row q-gutter-xs" style="width: 500px;">
        ...
        <q-card-actions align="right">
            <q-btn type="submit" :loading="submitting" @click.stop="createModelo" color="teal" label="Gravar" />
...
export default {
  ...
  methods: {
    filterFn (val, update, abort) {
      const stringOptions = this.marcas
      if (val === '') {
        update(() => {
