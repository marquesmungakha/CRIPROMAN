...
export default {
  ...
  methods: {
      ...
      if (this.editedIndex > -1) {
        ...
        this.addNewFuncao(this.funcao).then(resp => {
          this.$q.notify({
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi inserida com successo! [ ' + this.funcao.designacao + ' ]'
          })
          this.close()
        }).catch(error => {
          console.log(error)
    ...
    close () {
      ...
      setTimeout(() => {
        // this.funcao = Object.assign({}, null)
        this.editedIndex = -1
      }, 300)
    },
    removeFuncao (funcao) {
    ...
    exportTable () {
      ...
      const content = [this.columns.map(col => wrapCsvValue(col.label))]
        .concat(
          this.$store.state.funcao.funcaos.map(row =>
            this.columns
              .map(col =>
                wrapCsvValue(
        ...
        .join('\r\n')
      const status = exportFile('funcao_list.csv', content, 'text/csv')
      if (status !== true) {
        this.$q.notify({
