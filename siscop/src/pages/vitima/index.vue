<template>
  <q-page class="q-pa-sm q-gutter-sm">
    <q-table :columns="columns" :data="allVitimasFromPecaProcesso" :filter="filter" binary-state-sort
    ...
    <div class="q-pa-sm q-gutter-sm">
             ...
             <div class="q-pa-md">
              ...
              <individuo :apelido.sync="vitima.apelido"
                    ...
                    <q-step
                      ...
                      icon="assignment">
                      <create-edit-form :declaracao.sync="pecaProcessoVitima.declaracao"/>
                    </q-step>
     ...
     <details-vitima :vitima.sync="vitima" 
                       :image.sync="image" 
                       :pecaProcessoVitima.sync="pecaProcessoVitima" 
                       :tipoDocumento.sync="tipoDocumento"
                       :pais.sync="pais"
...
import PecaProcessoVitima from 'src/store/models/pecaProcesso/pecaProcessoVitima'
...
export default {
  ...
  metaInfo: {},
  computed: {
    allVitimasFromPecaProcesso() {
      return PecaProcesso.query().with('vitimas')
                                 .with('vitimas.vitima')
                                 ...
                                 .find(this.pecaProcesso.id).vitimas
    },
    allPaises() {
 ...
 //     }, 300)
      ...
      this.vitima.pecaProcesso_id = this.pecaProcesso.id
      this.vitima.nacionalidade = this.pais
      this.vitima.provincia = this.provincia
      this.vitima.tipoDocumento = this.tipoDocumento
      this.vitima.pecaProcesso = this.pecaProcesso

      this.pecaProcessoVitima.vitima =  this.vitima
      this.pecaProcessoVitima.pecaProcesso = this.pecaProcesso
     
      if (this.editedIndex > -1) {
         PecaProcessoVitima.api().patch("/pecaProcessoVitima/" + this.pecaProcessoVitima.id, this.pecaProcessoVitima).then(resp => {
        this.submitting = false
          this.$q.notify({
            type: 'positive',
            color: 'green-4',
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi actualizada com successo!! [' + this.pecaProcessoVitima.vitima.nome + ' ]'
          })
          this.close()
        }).catch(error => {
        ...
        PecaProcessoVitima.api().post("/pecaProcessoVitima/", this.pecaProcessoVitima).then(resp => {
        this.submitting = false
          console.log(resp)
          this.$q.notify({
            type: 'positive',
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi inserida com successo! [ ' + this.pecaProcessoVitima.vitima.nome + ' ]'
          })
          this.close()
        }).catch(error => {
          console.log('Erro no code ' + error)
    ...
    removeVitima(vitima) {
      this.$q.dialog({
        ...
        this.$q.notify({
         ...
         Vitima.api().delete("/pecaProcessoVitima/" + vitima.id)
         Vitima.delete(vitima.id)
      })
    },
    editaVitima(vitima) {
      this.editedIndex = 0
      this.pecaProcessoVitima = Object.assign({}, vitima)
      this.vitima =  this.pecaProcessoVitima.vitima
      this.pais = Pais.query().find(this.vitima.nacionalidade_id)
      this.provincia = Provincia.query().find(this.vitima.provincia_id)
      this.tipoDocumento = TipoDocumentoIdentificacao.query().find(this.vitima.tipoDocumento_id)
      this.image ='data:image/jpeg;base64,' + btoa(new Uint8Array(this.vitima.fotografia).reduce((data, byte) => data + String.fromCharCode(byte), ''))
    ...
    getAllVitima(offset) {
     ...
     getAllVitimaProcesso(offset) {
       if(offset >= 0){
          PecaProcessoVitima.api().get("/pecaProcessoVitima?offset="+offset+"&max=100").then(resp => {
        this.submitting = false
          offset = offset + 100
          if(resp.response.data.length > 0) 
  ...
  //   },
    ...
    exportTable() {
      ...
      const content = [this.columns.map(col => wrapCsvValue(col.label))]
        .concat(
          this.allVitimasFromPecaProcesso.map(row =>
            this.columns
              .map(col =>
                wrapCsvValue(
