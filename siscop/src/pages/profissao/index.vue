<template>
  ...
  <q-table title="Profissão" :data="allProfissaos" :columns="columns" row-key="name" binary-state-sort :filter="filter">
          ...
          <q-tr :props="props">
            ...
            <q-td key="actions" :props="props">
             <div class="q-gutter-sm">
              ...
              <q-btn round glossy icon="edit" color="blue" @click.stop="editaProfissao(props.row)" size=sm no-caps />
              <q-btn round glossy icon="delete_forever" color="red" @click.stop="removeProfissao(props.row)" size=sm no-caps/>
             </div>
            </q-td>
          </q-tr>
        </template>
...
export default {
  ...
  preFetch ({ store, currentRoute, previousRoute, redirect, ssrContext, urlPath, publicPath }) {
    ...
    // Example:
    return store.dispatch('profissao/getAllProfissao', currentRoute.params.id)
  },
  mounted () {
  },
  components: {
  ...
  computed: {
    ...mapGetters('profissao', ['allProfissao']),
    allProfissaos () {
      return this.$store.state.profissao.profissaos
    }
  },
  methods: {
    ...mapActions('profissao', ['getAllProfissao', 'addNewProfissao', 'updateProfissao', 'deleteProfissao']),
    createProfissao () {
      this.listErrors = []
      ...
      if (this.editedIndex > -1) {
        this.updateProfissao(this.profissao).then(resp => {
          console.log(resp)
          this.$q.notify({
            type: 'positive',
            color: 'green-4',
        ...
        this.addNewProfissao(this.profissao).then(resp => {
          this.$q.notify({
            type: 'positive',
            color: 'green-4',
            textColor: 'white',
    ...
    removeProfissao (profissao) {
      this.$q.dialog({
        ...
        this.deleteProfissao(profissao)
      })
    },
    editaProfissao (profissao) {
      this.editedIndex = this.$store.state.profissao.profissaos.indexOf(profissao)
    ...
    exportTable () {
      ...
      const content = [this.columns.map(col => wrapCsvValue(col.label))]
        .concat(
          this.$store.state.profissao.profissaos.map(row =>
            this.columns
              .map(col =>
                wrapCsvValue(
