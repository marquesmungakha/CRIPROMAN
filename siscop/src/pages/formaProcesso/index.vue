<template>
  ...
  <q-table title="Forma de Processo" :data="allFormaProcessos" :columns="columns" row-key="name" binary-state-sort :filter="filter">
          ...
          <q-tr :props="props">
            ...
            <q-td key="actions" :props="props">
             <div class="q-gutter-sm">
              ...
              <q-btn round glossy icon="edit" color="blue" @click.stop="editaFormaProcesso(props.row)" size=sm no-caps />
              <q-btn round glossy icon="delete_forever" color="red" @click.stop="removeFormaProcesso(props.row)" size=sm no-caps/>
             </div>
            </q-td>
          </q-tr>
        </template>
...
export default {
  ...
  preFetch ({ store, currentRoute, previousRoute, redirect, ssrContext, urlPath, publicPath }) {
    ...
    // Example:
    return store.dispatch('formaProcesso/getAllFormaProcesso', currentRoute.params.id)
  },
  mounted () {
  },
  components: {
  ...
  computed: {
    ...mapGetters('formaProcesso', ['allFormaProcesso']),
    allFormaProcessos () {
      return this.$store.state.formaProcesso.formaProcessos
    }
  },
  methods: {
    ...mapActions('formaProcesso', ['getAllFormaProcesso', 'addNewFormaProcesso', 'updateFormaProcesso', 'deleteFormaProcesso']),
    createFormaProcesso () {
      this.listErrors = []
      ...
      if (this.editedIndex > -1) {
        this.updateFormaProcesso(this.formaProcesso).then(resp => {
          console.log(resp)
          this.$q.notify({
            type: 'positive',
            color: 'green-4',
        ...
        this.addNewFormaProcesso(this.formaProcesso).then(resp => {
          this.$q.notify({
            type: 'positive',
            color: 'green-4',
            textColor: 'white',
    ...
    close () {
      this.$store.dispatch('formaProcesso/getAllFormaProcesso')
    ...
    editaFormaProcesso (formaProcesso) {
      this.editedIndex = this.$store.state.formaProcesso.formaProcessos.indexOf(formaProcesso)
