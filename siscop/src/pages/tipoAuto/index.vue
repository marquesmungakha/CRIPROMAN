<template>
  ...
  <q-table title="Tipo de Auto" :data="allTipoAutos" :columns="columns" row-key="name" binary-state-sort :filter="filter">
          ...
          <q-tr :props="props">
            ...
            <q-td key="actions" :props="props">
             <div class="q-gutter-sm">
              <router-link :to="`/tipoAuto/${props.row.id}`" >
              <q-btn round glossy icon="visibility" color="secondary" size=sm no-caps />
               </router-link>
              <q-btn round glossy icon="edit" color="blue" @click.stop="editaTipoAuto(props.row)" size=sm no-caps />
              <q-btn round glossy icon="delete_forever" color="red" @click.stop="removeTipoAuto(props.row)" size=sm no-caps/>
             </div>
            </q-td>
          </q-tr>
        </template>
  ...
  <create-edit-form :show_dialog="show_dialog"
                    ...
                    :codigo.sync="tipoAuto.codigo"
                    :designacao.sync="tipoAuto.designacao"
...
export default {
  ...
  preFetch ({ store, currentRoute, previousRoute, redirect, ssrContext, urlPath, publicPath }) {
    ...
    // Example:
    return store.dispatch('tipoAuto/getAllTipoAuto', currentRoute.params.id)
  },
  mounted () {
  },
  components: {
  ...
  computed: {
    ...mapGetters('tipoAuto', ['allTipoAuto']),
    allTipoAutos () {
      return this.$store.state.tipoAuto.tipoAutos
    }
  },
  methods: {
    ...mapActions('tipoAuto', ['getAllTipoAuto', 'addNewTipoAuto', 'updateTipoAuto', 'deleteTipoAuto']),
    createTipoAuto () {
      this.listErrors = []
      ...
      if (this.editedIndex > -1) {
        this.updateTipoAuto(this.tipoAuto).then(resp => {
          console.log(resp)
          this.$q.notify({
            type: 'positive',
            color: 'green-4',
        ...
        this.addNewTipoAuto(this.tipoAuto).then(resp => {
          this.$q.notify({
            type: 'positive',
            color: 'green-4',
            textColor: 'white',
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi inserida com successo! [ ' + this.tipoAuto.designacao + ' ]'
          })
          this.close()
        }).catch(error => {
          console.log(error)
    ...
    close () {
      ...
      setTimeout(() => {
        // this.tipoAuto = Object.assign({}, null)
        this.editedIndex = -1
      }, 300)
    },
    ...
    editaTipoAuto (tipoAuto) {
      this.editedIndex = this.$store.state.tipoAuto.tipoAutos.indexOf(tipoAuto)
    ...
    exportTable () {
      ...
      const content = [this.columns.map(col => wrapCsvValue(col.label))]
        .concat(
          this.$store.state.tipoAuto.tipoAutos.map(row =>
            this.columns
              .map(col =>
                wrapCsvValue(
        ...
        .join('\r\n')
      const status = exportFile('tipoAuto_list.csv', content, 'text/csv')
      if (status !== true) {
        this.$q.notify({
