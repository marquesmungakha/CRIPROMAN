          ...
          <q-card class="my-card" flat bordered>
            ...
            <q-card-section class="bg-white text-grey">
              <div class="row">
                ...
                <q-item class="full-width">
                  <q-item-section>
                    <q-item-label lines="1" caption >{{ $t('marca') }}</q-item-label>
                    <q-item-label class="text-grey-9">{{ getMarcaLocal.designacao }}</q-item-label>
                  </q-item-section>
                </q-item>
                <q-separator/>
              </div>
                ...
                <q-card-actions align="right">
                    <q-btn class="glossy" label="Editar" color="teal" @click.stop="editaModelo(modelo)" no-caps />
                    <q-btn class="glossy" label="Apagar" color="negative" @click.stop="removeModelo(modelo)" no-caps/>
                </q-card-actions>
            </q-card>
        </div>
        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
...
export default {
  ...
  preFetch ({ store, currentRoute, previousRoute, redirect, ssrContext, urlPath, publicPath }) {
    ...
    // Example:
    return store.dispatch('modelo/getModelo', currentRoute.params.id)
  },
  created () {
  },
  mounted () {
  ...
  computed: {
    modelo: {
      get () {
        return this.$store.getters['modelo/modelo']
      },
      set (modelo) {
        this.SET_UPDATE_MODELO({ modelo })
        this.$emit('update:modelo', '')
        this.$store.commit('modelo/SET_UPDATE_MODELO', modelo)
      }
    },
    allMarcas () {
    ...
    getMarcaLocal () {
      const localMarca = this.allMarcas.filter(marca => marca.id === this.modelo.marca.id)
  ...
  methods: {
    ...mapActions('modelo', ['getAllModelo', 'getModelo', 'addNewModelo', 'updateModelo', 'deleteModelo']),
    ...mapMutations('modelo', ['SET_UPDATE_MODELO']),
    removeModelo (modelo) {
      this.$q.dialog({
        title: 'Confirmação',
    ...
    createModelo () {
      ...
      this.localModelo.marca.id = this.marca.id
      this.updateModelo(this.localModelo).then(resp => {
