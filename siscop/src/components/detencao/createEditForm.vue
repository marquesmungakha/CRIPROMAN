<template>
    <div class="q-pa-sm q-gutter-sm">
       <q-dialog v-model="show_dialog" persistent>
        ...
        <q-card-section>
            <div class="text-h6">Adicionar Auto de Detenção!</div>
        </q-card-section>
                <q-card-section>
        ...
        <q-card-actions align="right">
            <q-btn type="submit" :loading="submitting" @click.stop="createDetencao" color="teal" label="Gravar" />
...
export default {
  ...
  methods: {
    filterFnMotivoDetencao (val, update, abort) {
      const stringOptions = this.motivoDetencaos
      if (val === '') {
        update(() => {
          this.optionsMotivoDetencao = stringOptions.map(motivoDetencao => motivoDetencao)
        })
      } else if (stringOptions.length === 0) {
        update(() => {
          this.optionsMotivoDetencao = []
        })
      } else {
        update(() => {
          this.optionsMotivoDetencao = stringOptions
            .map(motivoDetencao => motivoDetencao)
            .filter(motivoDetencao => {
              return motivoDetencao &&
                   motivoDetencao.designacao.toLowerCase().indexOf(val.toLowerCase()) !== -1
            })
        })
      }
    },
