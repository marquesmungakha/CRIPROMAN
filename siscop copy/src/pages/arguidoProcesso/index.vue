<template>
  <q-page class="q-pa-sm q-gutter-sm">
    <q-table :columns="columns" :data="allArguidosFromProcessoInstrucaoPreparatoria" :filter="filter" binary-state-sort
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td key="morada" :props="props">
            {{ props.row.arguido.morada }}
            <q-popup-edit v-model="props.row.arguido.morada">
              <q-input v-model="props.row.arguido.morada" autofocus counter dense></q-input>
            </q-popup-edit>
          </q-td>
...
export default {
  ...
  metaInfo: {},
  computed: {
    allArguidosFromProcessoInstrucaoPreparatoria() {
      return ProcessoInstrucaoPreparatoria.query().with('arguidos')
                                 ...
                                 .find(this.processoInstrucaoPreparatoria.id).arguidos
    },
    allPaises() {
  ...
  methods: {
    ...
    createArguido() {
      ...
      this.arguido.processoInstrucaoPreparatoria_id = this.processoInstrucaoPreparatoria.id
      ...
      if (this.editedIndex > -1) {
         ProcessoInstrucaoPreparatoriaArguido.api().patch("/processoInstrucaoPreparatoriaArguido/" + this.processoInstrucaoPreparatoriaArguido.id, this.processoInstrucaoPreparatoriaArguido).then(resp => {
          this.$q.notify({
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi actualizada com successo!! [' + this.processoInstrucaoPreparatoriaArguido.arguido.nome + ' ]'
          })
          this.close()
        }).catch(error => {
        ...
        ProcessoInstrucaoPreparatoriaArguido.api().post("/processoInstrucaoPreparatoriaArguido/", this.processoInstrucaoPreparatoriaArguido).then(resp => {
          ...
          this.$q.notify({
            ...
            classes: 'glossy',
            progress: true,
            message: 'A informação foi inserida com successo! [ ' + this.processoInstrucaoPreparatoriaArguido.arguido.nome + ' ]'
          })
          this.close()
        }).catch(error => {
          console.log('Erro no code ' + error)
